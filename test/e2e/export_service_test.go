...
var _ = Describe("Test exporting service", func() {
	...
	Context("When networking features are enabled", func() {
		...
		AfterEach(func() {
			if !enabledNetworkingFeatures {
				Skip("Skipping removing workload when networking features are not enabled")
			}
	...
	Context("When networking features are disabled", func() {
		...
		AfterEach(func() {
			if enabledNetworkingFeatures {
				Skip("Skipping removing workload when networking features are enabled")
			}
			By("Removing workload")
			Expect(wm.RemoveWorkload(ctx)).Should(Succeed())
		})

		It("should not allow creation of networking CRs", func() {
			By("Denying create service export due to missing CRD")
			Expect(wm.ExportService(ctx, wm.ServiceExport())).ShouldNot(Succeed())

			By("Denying create multi-cluster service due to missing CRD")
			Expect(wm.CreateMultiClusterService(ctx, wm.MultiClusterService())).ShouldNot(Succeed())
		})
	})
})
